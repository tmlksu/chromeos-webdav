...
**設定を変えるとき**は、同じ URL でもう一度追加する。共有の id は URL から
決定的に決まるので、同じ URL なら上書きになる (重複はできない)。
パスワードの変更もこの手順で行う。
...
- **すべてのリクエストに期限を張る**。無いと、トンネルやプロキシが応答を返さなくなったとき
  `fetch` が返らず、**Files アプリが無限に待つ** (ChromeOS 側から中断する手段が無い)。
  メタデータ (PROPFIND) は 30 秒、本体の読み出しは 120 秒 — 遅い回線を弾かないよう
  読み出し側を長く取っている。期限は**試行ごとに張り直す**。Cloudflare Access の
  再認証は最大 90 秒かかるので、ログインを挟んだ再試行に同じ期限を使い回すと必ず切れる。
- **経路の障害を設定ミスやバグと同じ扱いにしない**。5xx とタイムアウトは `IO`、
  401/403 は `ACCESS_DENIED`、404 は `NOT_FOUND`、405/501 は `INVALID_OPERATION`。
  全部 `FAILED` に潰すと「トンネルが落ちている」「認証が違う」「拡張のバグ」が
  ユーザから区別できない。写像は `errors.js` にあり、chrome API に触れないので
  そのままテストできる。
